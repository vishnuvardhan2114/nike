...
export async function getAllProducts(filters: NormalizedProductFilters): Promise<GetAllProductsResult> {
  ...
  const hasSize = filters?.sizeSlugs && filters.sizeSlugs.length > 0;
  const hasColor = filters?.colorSlugs && filters.colorSlugs.length > 0;
  ...
  if (hasSize) {
    variantConds.push(inArray(productVariants.sizeId, db
      .select({ id: sizes.id })
      .from(sizes)
      .where(filters.sizeSlugs ? inArray(sizes.slug, filters.sizeSlugs as string[]) : undefined)));
  }
  if (hasColor) {
    variantConds.push(inArray(productVariants.colorId, db
      .select({ id: colors.id })
      .from(colors)
      .where(filters.colorSlugs ? inArray(colors.slug, filters.colorSlugs as string[]) : undefined)));
  }
  if (hasPrice) {
    ...
    ? db
        ...
        .where(
          inArray(
            ...
            db.select({ id: colors.id }).from(colors).where(filters.colorSlugs ? inArray(colors.slug, filters.colorSlugs as string[]) : undefined)
          )
        )
        .as("pi")
    : db
